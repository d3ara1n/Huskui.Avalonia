...
public sealed class PageCoverOverTransition : PageTransitionBase
{
    ...
    public PageCoverOverTransition(TimeSpan? duration = null, DirectionFrom? direction = null)
        : base(duration) => Direction = direction ?? DirectionFrom.Right;
    ...
    protected override void Configure(Builder from, Builder to, Lazy<Visual> parentAccessor)
    {
        ...
        to.Animation(new BackEaseOut())
            .AddFrame(
                0.5d,
                [(ScaleTransform.ScaleXProperty, 0.98d), (ScaleTransform.ScaleYProperty, 0.98d)]
            )
            .AddFrame(
                1d,
        ...
        to.Animation(new SplineEasing(0.1, 0.9, 0.2))
