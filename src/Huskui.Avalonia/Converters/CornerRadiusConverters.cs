...
public static class CornerRadiusConverters
{
    ...
    public static IValueConverter ToDouble { get; } =
        new RelayConverter(
            (v, p) =>
            {
    ...
    public static IValueConverter FromDouble { get; } =
        new RelayConverter(
            (v, _) =>
            {
                return v switch
    ...
    public static IValueConverter ToInnerRadius { get; } =
        new RelayConverter(
            (v, p) =>
            {
                if (v is CornerRadius radius)
                {
                    ...
                    if (padding.HasValue)
                    {
                        return new CornerRadius(
                            radius.TopLeft
                                - padding.Value / (radius.TopLeft / padding < 1.5 ? 3d : 1d),
                            radius.TopRight
                                - padding.Value / (radius.TopRight / padding < 1.5 ? 3d : 1d),
                            radius.BottomRight
    ...
    public static IMultiValueConverter ToInnerRadiusMulti { get; } =
        new RelayMultiConverter(
            (v, _, _) =>
            {
                var (radius, padding) = v switch
                {
                    ...
                    [CornerRadius c, string p] when double.TryParse(p, out var result) => (
                        c,
                        result
                    ),
                ...
                return new CornerRadius(
                    radius.TopLeft - padding / (radius.TopLeft / padding < 1.5 ? 3d : 1d),
