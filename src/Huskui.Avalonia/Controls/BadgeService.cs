...
public sealed class BadgeService : AvaloniaObject
{
    ...
    public static readonly AttachedProperty<object?> ContentProperty =
        AvaloniaProperty.RegisterAttached<BadgeService, Control, object?>("Content");

    public static readonly AttachedProperty<IDataTemplate?> ContentTemplateProperty =
        AvaloniaProperty.RegisterAttached<BadgeService, Control, IDataTemplate?>("ContentTemplate");

    public static readonly AttachedProperty<string?> ClassesProperty =
        AvaloniaProperty.RegisterAttached<BadgeService, Control, string?>("Classes");

    public static readonly AttachedProperty<BadgePlacement> PlacementProperty =
        AvaloniaProperty.RegisterAttached<BadgeService, Control, BadgePlacement>("Placement", BadgePlacement.TopRight);

    private static readonly AttachedProperty<State?> StateProperty =
        AvaloniaProperty.RegisterAttached<BadgeService, Control, State?>("State");
    ...
    public static void SetContent(Control element, object? value) => element.SetValue(ContentProperty, value);

    public static IDataTemplate? GetContentTemplate(Control element) => element.GetValue(ContentTemplateProperty);
    ...
    private static string[] GetClassNames(Control control) =>
        Classes.Parse(GetClasses(control) ?? string.Empty).Where(x => !string.IsNullOrWhiteSpace(x)).ToArray();
